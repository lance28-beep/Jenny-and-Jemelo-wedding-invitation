...
function MessageForm({ onSuccess, onMessageSent }: MessageFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await fetch(
        "https://docs.google.com/forms/d/e/1FAIpQLSc_4JPGXN8Kf7sneqkAIwEYnmDsVZwqQcDodfN3B2KZ1WlKEg/formResponse",
        {
          method: "POST",
          mode: "no-cors",
          body: googleFormData,
  ...
  return (
    <div className="relative w-full max-w-md mx-auto px-3 sm:px-0">
      ...
      <div className="absolute -top-3 -left-3 w-8 h-8 bg-[#A4AD92]/25 rounded-full blur-sm animate-pulse-slow" />
      <div className="absolute -bottom-4 -right-4 w-12 h-12 bg-[#848B69]/30 rounded-full blur-md animate-pulse-slow" />
      
      <Card className={`relative w-full border-2 border-[#A4AD92]/70 shadow-[0_12px_30px_rgba(132,139,105,0.35)] bg-white/60 backdrop-blur-md transition-all duration-500 group overflow-hidden rounded-2xl ${
        isFocused ? 'scale-[1.01] border-[#848B69] bg-white/75' : 'hover:bg-white/70'
      } ${isSubmitted ? 'animate-bounce' : ''}`}>
        {/* Glass effect gradient overlays */}
        <div className="absolute inset-0 bg-gradient-to-br from-[#A4AD92]/22 via-white/10 to-transparent" />
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-[#A4AD92]/28 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000" />
        
        {/* Success animation overlay */}
        {isSubmitted && (
          <div className="absolute inset-0 bg-gradient-to-br from-[#E8BBB8]/25 via-[#F3D8DD]/20 to-transparent flex items-center justify-center z-20 pointer-events-none">
            <div className="flex flex-col items-center gap-2 animate-pulse">
              <div className="w-16 h-16 bg-[#E8BBB8] rounded-full flex items-center justify-center shadow-lg">
        ...
        <CardContent className="relative p-3 sm:p-5 md:p-6 lg:p-8 xl:p-10">
          ...
          <div className="text-center mb-3 sm:mb-4 md:mb-5 lg:mb-6">
            <div className="relative inline-block mb-2 sm:mb-3 md:mb-4">
              <div className="absolute inset-0 bg-[#A4AD92]/40 rounded-full blur-lg scale-150" />
              <div className="relative w-9 h-9 sm:w-11 sm:h-11 md:w-14 md:h-14 bg-[#848B69] rounded-full flex items-center justify-center mx-auto shadow-lg">
          ...
          >
            ...
            <div className="space-y-1.5 sm:space-y-2 md:space-y-3">
              <label className="block text-xs sm:text-sm md:text-base font-medium text-foreground font-lora flex items-center gap-1.5 sm:gap-2">
                <div className={`w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 bg-gradient-to-br from-[#A4AD92]/25 to-[#848B69]/25 rounded-full flex items-center justify-center transition-all duration-300 ${
                  focusedField === 'name' ? 'scale-110 bg-[#A4AD92]/40' : ''
                }`}>
                  <Heart className="h-2.5 w-2.5 sm:h-3 sm:w-3 md:h-4 md:w-4 text-[#848B69]" />
                </div>
                Your Name
              </label>
              <div className="relative">
                <Input
                  ...
                  className={`message-form-input w-full border-2 rounded-xl py-2 sm:py-2.5 md:py-3 lg:py-3.5 px-3 sm:px-4 md:px-5 text-xs sm:text-sm md:text-base font-lora placeholder:italic transition-all duration-300 bg-white/85 backdrop-blur-sm shadow-sm hover:shadow-md focus:shadow-lg ${
                    focusedField === 'name' 
                      ? 'border-[#848B69] focus:border-[#848B69] focus:ring-4 focus:ring-[#A4AD92]/25 shadow-lg' 
                      : 'border-[#A4AD92]/40 hover:border-[#A4AD92]/40'
                  }`}
                />
                {nameValue && (
            ...
            <div className="space-y-1.5 sm:space-y-2 md:space-y-3">
              <div className="flex items-center justify-between">
                <label className="block text-xs sm:text-sm md:text-base font-medium text-foreground font-lora flex items-center gap-1.5 sm:gap-2">
                  <div className={`w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 bg-gradient-to-br from-[#A4AD92]/25 to-[#848B69]/25 rounded-full flex items-center justify-center transition-all duration-300 ${
                    focusedField === 'message' ? 'scale-110 bg-[#A4AD92]/40' : ''
                  }`}>
                    <MessageCircle className="h-2.5 w-2.5 sm:h-3 sm:w-3 md:h-4 md:w-4 text-[#848B69]" />
                  </div>
                  Your Message
                </label>
                {messageValue && (
            ...
            <Button
              ...
              className="w-full text-white py-2 sm:py-2.5 md:py-3 lg:py-3.5 px-4 sm:px-5 md:px-6 lg:px-7 rounded-xl text-xs sm:text-sm md:text-base font-lora font-semibold backdrop-blur-sm transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed disabled:transform-none relative overflow-hidden group border border-[#A4AD92]"
              style={{ 
                backgroundColor: "#848B69",
                boxShadow: "0 6px 20px rgba(132,139,105,0.35), 0 2px 8px rgba(132,139,105,0.2)",
              }}
              onMouseEnter={(e) => {
                if (!e.currentTarget.disabled) {
                  e.currentTarget.style.backgroundColor = "#A4AD92";
                  e.currentTarget.style.boxShadow = "0 8px 24px rgba(132,139,105,0.4), 0 3px 10px rgba(164,173,146,0.25)";
                }
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.backgroundColor = "#848B69";
                e.currentTarget.style.boxShadow = "0 6px 20px rgba(132,139,105,0.35), 0 2px 8px rgba(132,139,105,0.2)";
              }}
            >
              {isSubmitting ? (
        ...
        <div className="relative max-w-4xl mx-auto">
          <div className="text-center mb-4 sm:mb-6 md:mb-8">
            <div className="relative inline-block mb-3 sm:mb-4 md:mb-6">
              <div className="absolute inset-0 bg-gradient-to-r from-[#848B69]/50 via-[#A4AD92]/35 to-[#848B69]/30 rounded-full blur-xl scale-150 animate-pulse-slow" />
              <div className="relative w-8 h-8 sm:w-10 sm:h-10 md:w-14 md:h-14 bg-gradient-to-br from-[#848B69] via-[#A4AD92] to-[#848B69] rounded-full flex items-center justify-center mx-auto shadow-lg hover:scale-110 transition-transform duration-300">
                <MessageCircle className="h-4 w-4 sm:h-6 sm:h-6 md:h-8 md:w-8 text-[#FDFDFD]" />
              </div>
              {/* Outer glow ring */}
              <div className="absolute -inset-2 rounded-full bg-gradient-to-br from-[#848B69]/35 via-[#A4AD92]/25 to-[#848B69]/25 blur-md opacity-0 hover:opacity-100 transition-opacity duration-300" />
            </div>
